...
- (CGFloat)randomInRangeOf:(CGFloat)range {
    CGFloat amplification = 5000.0;
    NSInteger modulus = (int)(range * 2.0 * amplification);
    CGFloat preprocessed = arc4random() % modulus;
    CGFloat centeringFactor = range * amplification;
    preprocessed -= centeringFactor;
    CGFloat result = preprocessed / amplification;
    return result;
}
...
- (void)resetPlayer {
    ...
    CGFloat maxDelta = viewExtents.width / 256.0;
...
- (void)placeGravitySource:(CGPoint)sourcePoint {
    ...
    CGFloat sunRadius = (viewExtents.width / 15.0);
    UIView* sun = [[UIView alloc] initWithFrame:CGRectMake(0.0, 0.0, sunRadius * 2.0, sunRadius * 2.0)];
    sun.backgroundColor = [UIColor yellowColor];
    sun.layer.cornerRadius = sunRadius;
...
- (void)viewDidLoad
{
    ...
    CGFloat playerRadius = (viewExtents.width / 64.0);
    self.player = [[UIView alloc] initWithFrame:CGRectMake(0.0, 0.0, playerRadius * 2.0, playerRadius * 2.0)];
    self.player.backgroundColor = [UIColor greenColor];
    self.player.layer.cornerRadius = playerRadius;
...
- (void)mainClockTick:(NSTimer *)timer {
    ...
    CGFloat buffer = 100.0;
    
    if (center.x < -buffer || 
        center.y < -buffer ||
        center.y > viewExtents.width + buffer || 
        center.x > viewExtents.height + buffer) {
        [self resetPlayer];
    }
}
